...
export default function Message({ message, own, onClickDelete, userId, onClickDeleteMgsUser, onClickDeleteMgsFri, avatar }) {
  ...
  //   const getUser = async () => {
  //     try {
  //       const res = await axios("http://18.142.139.131:8800/api/users/name?userId="+message.sender);  
  //       setUser(res.data);
  //     } catch (err) {
  //       console.log(err); 
  //     }
  ...
  const handleDeleteMessage = async () => {
    try {
      ...
      const res = await axios.put("http://18.142.139.131:8800/api/messages/recall", { "id": message._id });
  ...
  const handleDeleteMgsUser = async () => {
    try {
      ...
      const res = await axios.put("http://18.142.139.131:8800/api/messages/del", data);
      console.log(res.data);
  ...
  const handleDeleteMgsFri = async () => {
    try {
      ...
      await axios.put("http://18.142.139.131:8800/api/messages/del", data);
